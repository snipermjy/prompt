...
export async function generateMetadata(
  ...
  技术开发 > 产品设计: 【仅限软件/互联网产品】产品需求、原型设计、用户研究、产品规划
  ...
  效率工具 > 学习教育: 【仅限教育工作者】教学内容设计、课程开发、教案编写
  效率工具 > 自动化工具: 脚本编写、工作流自动化、批处理
  
  AI应用 > AI智能体: 角色扮演、通用对话、个人咨询辅导（职业/心理/生活）
  `;
...
${categoryStructure}
   ...
   - 分类判断原则：
     * 看提示词的**最终产出**和**应用场景**，而不是形式
     ...
     【角色扮演类提示词的分类规则】：
     * 如果角色有明确的专业领域产出 → 按产出分类
       例如：UI设计师角色 → 产出设计稿 → "内容创作 > 设计创意"
       例如：文案写作角色 → 产出文案 → "内容创作 > 文字创作"
       例如：Python导师角色 → 产出代码教学 → "技术开发 > 代码开发"
     
     * 如果角色是通用对话/咨询/辅导类 → 分到"AI应用 > AI智能体"
       例如：人生导师、心理咨询师、职业顾问、生活助手 → "AI应用 > AI智能体"
       例如：通用助手、聊天机器人 → "AI应用 > AI智能体"
     ...
     * 如果角色用于特定业务场景 → 按业务分类
     ...
     【关键边界说明】：
     * "产品设计"仅限于：软件产品、APP、网站、互联网产品的设计规划
     * "学习教育"仅限于：教学内容设计、课程开发、教育工作者使用的工具
     * 个人咨询/辅导类（职业规划、心理咨询、生活指导）→ 统一归入"AI智能体"
   ...
   - 如果现有分类都不合适，可以创建新分类：
     ...
     【判断是否需要新分类】：
     满足以下任一条件，才创建新分类：
     ✅ 提示词涉及的领域在现有分类中完全没有覆盖
        例如：法律咨询、医疗健康、金融理财、游戏娱乐
     
     ✅ 提示词的应用场景与现有分类描述差异很大
        例如：现有"学习教育"是给老师用的，但提示词是给学生用的
     
     ✅ 强行归入现有分类会让用户困惑
        例如："股票分析师"放到"数据分析"还是"商业分析"都不合适
     
     ❌ 不要因为细微差异就创建新分类
        例如："React开发"不需要单独分类，归入"代码开发"即可
        例如："小说写作"不需要单独分类，归入"文字创作"即可
     
     【创建新分类的规则】：
     * 判断应该属于哪个一级分类（内容创作/技术开发/商业运营/效率工具/AI应用）
     * 如果5个一级分类都不合适 → 归入"AI应用 > AI智能体"（兜底）
